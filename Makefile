...
ifeq ($(OS),Windows_NT)
	compilation_flags := -Wall -std=c++17 -O3 -march=native -mtune=native -pthread -static-libgcc -static-libstdc++ -Ilibdatachannel/include -Llibdatachannel/build `pkg-config --cflags sdl2 gstreamer-video-1.0 gstreamer-app-1.0 gstreamer-1.0 nice gio-2.0`
	libraries := -lfltk -ldatachannel -lssl -lcrypto -lws2_32 `pkg-config --libs sdl2 gstreamer-video-1.0 gstreamer-app-1.0 gstreamer-1.0 nice gio-2.0`
	static_libraries :=
endif
